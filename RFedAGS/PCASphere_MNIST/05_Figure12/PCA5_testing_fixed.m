function [fvs, times, numgfs, problem] = PCA5_testing_fixed(iidtype, x0, K, method, option, s, Seed, stepsize, pi)

    if method == "RFedAGS"
        RFedMethod = @RFedAGSP;
    elseif method == "RFedAvg"
        RFedMethod = @RFedAvg_;
    elseif method == "RFedSVRG"
        RFedMethod = @RFedSVRG_;
    elseif method == "RFedProj"
        RFedMethod = @RFedProj_;
    else 
        error('error: not such method:%s.\n', method);
    end
    ...
    data_ = data;
    [problem, options] = construct_prob(data_, problem, options, nagents, ncostterms, ceil(ncostterms * 0.5), K, pi);    
    ...
    for seed = 1:Seed
        fprintf('seed:%d, K:%d\n', seed, K)
        % profile on
        [x, fv, info] = RFedMethod(problem, x0, options);
        % profile viewer
        fvs = [fvs, info.fvs];
