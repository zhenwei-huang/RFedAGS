...
data = zeros(200000, d);
for i = 1:100
    datatmp = (i / 100) * randn(2000, d);
    data((i-1)*2000 + 1 : i*2000, :) = datatmp;
end
...
filename = "PCA_Stief_Syn_train_localdatasizeincreas";
...
for seed = 1:Seed
    fprintf('seed:%d, K:%d, ncostterms:%d\n', seed, K1, ncostterms);
    % profile on
...
legend('S=400', 'S=800', 'S=1200', 'S=1600', 'S=2000', 'FontSize', 18)
...
function [problem, options] = construct_prob(D, problem, options, nagents, ncostterms, batchsize, K, pi)    
    ...
    D = partition_data(D, nagents, ncostterms);
...
function output = partition_data(data, nagents, ncostterms)
    output = zeros(nagents*ncostterms, size(data, 2));

    N = 2000;
    for i = 1:nagents
        datatmp = data((i-1) * N + 1 : i * N, :);
        output((i - 1) * ncostterms + 1 : i * ncostterms, :) = datatmp(1:ncostterms, :);
    end    
end
