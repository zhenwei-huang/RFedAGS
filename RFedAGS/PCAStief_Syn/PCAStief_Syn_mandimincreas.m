...
data = zeros(50000, 4000);
for i = 1:50
    datatmp = (i / 50) * randn(1000, 4000);
    data((i-1)*1000 + 1 : i*1000, :) = datatmp;
end
...
nagents = 50; ncostterms = 100;

filename = "PCA_Stief_Syn_train_mandimincreas";
...
problem.M = Manifold;
x0 = Manifold.rand();
[problem, options] = construct_prob(data, d, problem, options, nagents, ncostterms, ceil(ncostterms * 0.5), K1, pi);
...
title("$(N,S) = (50,1000)$", 'Interpreter','latex','FontSize',18)
ylabel('Optimality gap', 'FontSize',18,'Interpreter','latex')
xlabel('Iterations','FontSize',18,'Interpreter','latex')
legend('(d,r)=(1000,5)', '(d,r)=(2000,5)', '(d,r)=(2000,10)', '(d,r)=(4000,5)', '(d,r)=(4000,10)', 'FontSize', 18)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [problem, options] = construct_prob(D, d, problem, options, nagents, ncostterms, batchsize, K, pi)    

    D = partition_data(D, d);
...
function output = partition_data(data, d)    
    output = data(:,1:d);
end
